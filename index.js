...
const API_KEY = process.env.API_KEY || "replace-this-api-key";
const CLIENT_KEY = process.env.CLIENT_KEY || "replace-this-client-key";
...
const requireReadKey = (req, res, next) => {
  const userKey = req.header("x-user-key") || req.query.user_key; // Allow user_key in query for specific history
...
app.post("/api/ingest", requireWriteKey, async (req, res) => {
  ...
  const writeMemory = () => { /* ... existing memory write logic ... */ };
